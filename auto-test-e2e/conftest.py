...
MOCK_LLM_RESPONSES = {
    ...
    "review": {
        "quality_score": 85,
        "intent_match": 90,
        "suggestions": ["建议在 goto 后增加 wait_element 步骤"],
        "passed": True,
    },
}
...
class MockLLMHandler(BaseHTTPRequestHandler):
    ...
    def _pick_response(self, body: dict):
        ...
        if "审查" in system_msg or "review" in system_msg.lower() or "quality_score" in system_msg:
            return MOCK_LLM_RESPONSES["review"]
...
def api_client(api_token):
    """带认证的 requests Session (super_admin)"""
    import requests
...
@pytest.fixture(scope="session")
def guest_api_token():
    """获取 guest1 的 API Token"""
    import requests
    resp = requests.post(
        f"{API_URL}/api/auth/login/",
        json=TEST_USERS["guest"],
        timeout=10,
    )
...
@pytest.fixture(scope="session")
def guest_api_client(guest_api_token):
    """带认证的 requests Session (guest)"""
    import requests
    session = requests.Session()
    session.headers.update({
        "Authorization": f"Token {guest_api_token}",
        "Content-Type": "application/json",
    })
    session.base_url = f"{API_URL}/api"
    return session
